...
namespace MageMe\WebFormsFreshdesk\Observer;
...
class WebformsFormImportObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        $oldId = $form->getFreshdeskTicketEmailFieldId();
        if ($oldId && !empty($elementMatrix['field_' . $oldId])) {
            $form->setFreshdeskTicketEmailFieldId($elementMatrix['field_' . $oldId]);
        }
        $serializedFields = $form->getFreshdeskTicketMapFieldsSerialized();
        if ($serializedFields) {
            ...
            $form->setFreshdeskTicketMapFields($map);
        }
